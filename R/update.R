...
#' @param x A \code{lts}, \code{gts} or \code{imu} object
...
#' attr(gts3, 'name')
update.lts = function(x, type, new, keep.start = T){
  ...
  update.obj(x, type, new, keep.start)
}

#' @rdname update.lts
#' @export
update.gts = function(x, type, new, keep.start = T){
...
#' @rdname update.lts
#' @export
update.imu = function(x, type, new){
  if(! type%in%c('axis', 'freq', 'unit', 'name') ){
    stop("For imu object, you can only update 'axis', 'freq', 'unit', 'name'.")
  }
  update.obj(x, type, new)
}

#' @title Internal Function to Update the Attributes of Objects
#' @keywords internal
update.obj = function(x, type, new, keep.start = T){
  ...
  if(type == 'start'){
    ...
    if(keep.start){
      ...
      attr(x, 'freq') = new
      N = nrow(x)
      attr(x, 'end') = start + (N - 1)/new
    }else{
      end = attr(x, 'start')
      attr(x, 'freq') = new
      N = nrow(x)
      attr(x, 'start') = end - (N - 1)/new
    }
