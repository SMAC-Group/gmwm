...
modwt = function(x, nlevels = floor(log2(length(x))), filter = "haar", boundary="periodic", bw = TRUE) {
  out = .Call('gmwm_modwt_cpp', PACKAGE = 'gmwm', x, filter_name = filter, nlevels, boundary = boundary)
  names(o) = paste0("S",1:attr(o,"J"))
...
summary.modwt=function(object, ...){
  ...
  cat("These values are", if(!object$brick.wall){" >NOT<"}," brick walled\n")
  print.modwt(object)
}